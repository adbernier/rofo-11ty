...
const { searchMarketIdentity } = require("../search-intelligence/market-identity");
...
function normalizeSearchConsoleRecords(snapshot = searchConsoleSnapshot, options = {}) {
  ...
  return (snapshot.records || []).map((record) => {
    const identity = searchMarketIdentity({ state: record.state, marketId: record.legacyMarketId || record.marketId, marketName: record.marketName });
    ...
    return {
      ...record,
      marketId: record.marketKey ? record.marketId : identity.marketId,
      marketKey: record.marketKey || identity.marketKey,
      legacyMarketId: record.legacyMarketId || identity.legacyMarketId,
...
function buildMarketOpportunity(record, publisherAnalysis) {
  ...
  const coverage = marketCoverage(record.legacyMarketId || record.marketId, publisherAnalysis);
  ...
  return {
    ...
    marketKey: record.marketKey || null,
    legacyMarketId: record.legacyMarketId || record.marketId,
...
function buildTopicIntelligence(records, marketOpportunities) {
  const identityFor = (record) => record.marketKey || record.marketId;
  const marketById = new Map(marketOpportunities.map((market) => [identityFor(market), market]));
  const topicMap = new Map();

  records.forEach((record) => {
    const market = marketById.get(identityFor(record));
    (record.queries || []).forEach((query) => {
      ...
      intents.forEach((intent) => {
        ...
        if (identityFor(record)) {
          if (!topic.marketMap.has(identityFor(record))) {
            topic.marketMap.set(identityFor(record), {
              marketId: record.marketId,
              marketKey: record.marketKey || null,
              legacyMarketId: record.legacyMarketId || record.marketId,
              marketName: record.marketName,
          ...
          const topicMarket = topic.marketMap.get(identityFor(record));
...
function topMarketEvidence(markets, count = 4) {
  return (markets || []).slice(0, count).map((market) => ({
    ...
    legacyMarketId: market.legacyMarketId || market.marketId,
  ...
  propertyTypeConfigs.forEach((config) => {
    ...
    const combined = alternate && topic
      ? {
        ...
        marketCount: new Set([...(topic.strongestMarkets || []), ...(alternate.strongestMarkets || [])].map((market) => market.marketKey || market.marketId)).size,
        strongestMarkets: [...(topic.strongestMarkets || []), ...(alternate.strongestMarkets || [])]
          .sort((a, b) => (Number(b.impressions) || 0) - (Number(a.impressions) || 0))
          .filter((market, index, array) => array.findIndex((candidate) => (candidate.marketKey || candidate.marketId) === (market.marketKey || market.marketId)) === index),
