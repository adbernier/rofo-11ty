...
function campaignForProgram(metro, program, progress, missions, initiatives) {
...
function buildMarketProjection(metros, portfolioQueues, platformServices) {
  ...
  const markets = (metros || []).map((metro) => {
    const programs = Object.values(MARKET_PROGRAMS).map((program) => {
      ...
      const campaign = campaignForProgram(metro, program, progress, missions, initiatives);
      flatCampaigns.push({
        ...campaign,
...
function buildEditorialOperatingSystem(publisherSnapshot, expansionPlans, options = {}) {
  ...
  const marketProjection = buildMarketProjection(operationalMarkets, portfolioQueues, platformServices);
