...
const OPERATIONAL_MARKETS = [
  { id: "san-francisco", name: "San Francisco", state: "CA", cities: ["San Francisco"], publisherMetroId: "san-francisco" },
  { id: "east-bay", name: "East Bay", state: "CA", cities: ["Oakland", "Berkeley", "Emeryville", "Hayward", "Union City", "Fremont"], publisherMetroId: "san-francisco" },
  { id: "peninsula", name: "Peninsula", state: "CA", cities: ["Palo Alto", "Redwood City", "San Mateo"], publisherMetroId: "san-francisco" },
  { id: "south-bay", name: "South Bay", state: "CA", cities: ["Mountain View", "Sunnyvale", "San Jose"], publisherMetroId: "san-francisco" },
  { id: "sacramento", name: "Sacramento", state: "CA", cities: ["Sacramento", "West Sacramento", "Rancho Cordova", "Folsom", "Roseville", "Rocklin", "Elk Grove", "Davis"], publisherMetroId: "sacramento" },
  { id: "san-diego", name: "San Diego", state: "CA", cities: ["San Diego", "Carlsbad", "Oceanside", "Vista", "San Marcos", "Escondido", "Encinitas", "Del Mar", "Poway", "Santee", "Chula Vista"], publisherMetroId: "san-diego" },
  { id: "orange-county", name: "Orange County", state: "CA", cities: ["Irvine", "Newport Beach", "Costa Mesa", "Santa Ana", "Anaheim", "Tustin", "Lake Forest", "Mission Viejo", "Laguna Hills", "Huntington Beach", "Fullerton", "Orange"], publisherMetroId: "orange-county" },
  { id: "los-angeles", name: "Los Angeles", state: "CA", cities: ["Los Angeles", "Burbank", "Glendale", "Culver City", "Santa Monica", "El Segundo", "Torrance", "Long Beach", "Carson", "Commerce", "Vernon", "City of Industry", "Santa Fe Springs", "Pasadena"], publisherMetroId: "" },
  { id: "denver", name: "Denver", state: "CO", cities: ["Denver", "Boulder", "Aurora", "Lakewood", "Golden", "Arvada", "Westminster", "Broomfield", "Louisville", "Superior", "Longmont", "Thornton", "Northglenn", "Glendale", "Englewood", "Centennial", "Commerce City", "Lone Tree"], publisherMetroId: "denver" },
  { id: "seattle", name: "Seattle", state: "WA", cities: ["Seattle", "Bellevue", "Redmond", "Kirkland", "Kent"], publisherMetroId: "seattle" },
];

const OPERATIONAL_MARKET_BY_ID = new Map(OPERATIONAL_MARKETS.map((market) => [market.id, market]));
const OPERATIONAL_MARKET_BY_CITY = new Map();
for (const market of OPERATIONAL_MARKETS) {
  for (const city of market.cities || []) {
    OPERATIONAL_MARKET_BY_CITY.set(`${market.state}|${String(city).toLowerCase()}`, market);
  }
}
...
function operationalMarketForBuilding(buildingRecord, representativeRecord, fallbackMetroId, fallbackMetroName) {
  ...
  const market = OPERATIONAL_MARKET_BY_CITY.get(`${state}|${String(city).toLowerCase()}`);
  if (market) return { id: market.id, name: market.name };
  const fallback = OPERATIONAL_MARKET_BY_ID.get(fallbackMetroId);
  ...
  return {
    ...
    programs: {
      ...
      commercialMarketEvidence: {
        ...
        behavior: "One district collection is one valid portfolio Mission; evidence records remain hidden Work Items.",
      },
    },
  };
}
...
function buildBuildingProfilePortfolioMissions(portfolioResolution, metroRecords) {
  const portfolios = (((portfolioResolution || {}).programs || {}).buildingProfiles || {}).portfolios || [];
  return portfolios
    .filter((portfolio) => portfolio.eligibleForExecution)
...
function buildPortfolioQueues(workQueue, metros, expansionProjects, portfolioResolution) {
  ...
  const buildingProfileMissions = buildBuildingProfilePortfolioMissions(portfolioResolution, metros);
  const reservedWorkItemIds = new Set(buildingProfileMissions.flatMap((mission) => mission.includedOpportunityIds || []));
...
function buildCommercialMarketEvidenceMissions(metros, platformServices) {
  ...
  for (const metro of metros || []) {
    const nextDistrict = suggested.find((district) => district.marketId === metro.metroId || district.metroId === metro.metroId);
...
function buildOperationalMarketRecords(metros, platformServices) {
  const marketsById = new Map((metros || []).map((metro) => [metro.metroId, metro]));
...
function campaignSizingStrategy(program) {
  if (program.id === MARKET_PROGRAMS.commercialMarketEvidence.id) {
    return "District collections are Standard missions. Each mission hides evidence-record research, narrative, source selection, validator fixes, and minimal documentation work.";
  }
...
function campaignForProgram(metro, program, progress, missions, initiatives, portfolioResolution) {
  ...
  return {
    ...
    workItemCount: buildingProfileResolution ? buildingProfileResolution.hiddenWorkItemCount : workItemCount,
    ...
    workItems: {
      hiddenByDefault: true,
      count: buildingProfileResolution ? buildingProfileResolution.hiddenWorkItemCount : workItemCount,
    },
...
function buildMarketProjection(metros, portfolioQueues, platformServices, portfolioResolution) {
  ...
  return {
    ...
    hierarchy: ["Markets", "Programs", "Campaigns", "Initiatives", "Missions", "Execution Packets", "Work Items"],
...
function collectionOperationalMarket(collection) {
  ...
  if (marketId && OPERATIONAL_MARKET_BY_ID.has(marketId)) return OPERATIONAL_MARKET_BY_ID.get(marketId);
  return null;
}
...
function cityOperationalMarket(district) {
  const key = `${district.state || ""}|${String(district.city || "").toLowerCase()}`;
  return OPERATIONAL_MARKET_BY_CITY.get(key) || null;
}

function publisherOperationalMarket(publisherMetro) {
  if (!publisherMetro || !OPERATIONAL_MARKET_BY_ID.has(publisherMetro.metroId)) return null;
  return OPERATIONAL_MARKET_BY_ID.get(publisherMetro.metroId);
}
...
function buildCommercialMarketEvidenceExpansion(publisherAnalysis) {
  ...
  const operationalMarkets = Array.from(new Map(resolvedDistricts.map((district) => [district.marketId, {
    marketId: district.marketId,
    marketName: district.marketName,
    publisherMetroId: district.publisherMetroId,
    publisherMetroName: district.publisherMetroName,
    ...
    .map((market) => {
      const marketDistricts = resolvedDistricts.filter((district) => district.marketId === market.marketId);
      return {
        ...market,
...
function buildEditorialOperatingSystem(publisherSnapshot, expansionPlans, options = {}) {
  ...
  const programMissions = buildCommercialMarketEvidenceMissions(operationalMarkets, platformServices);
  const portfolioQueues = attachProgramMissions(buildPortfolioQueues(workQueue, metros, expansionProjects, portfolioResolution), programMissions);
