...
function buildDistrictBuildingEvidenceResolution(portfolioResolution, platformServices) {
  ...
  const collectionDistricts = collections.map((collection) => {
    ...
    return {
      ...
      eligibleForExecution: executable,
      groupingRationale: [
  ...
  const suggested = ((expansion && expansion.suggestedExpansionOrder) || [])
    ...
    .map((district) => {
      ...
      return {
        evidenceMissionId: `district-building-evidence:${district.marketId || district.metroId}:${district.districtId}`,
        marketId: district.marketId || district.metroId,
