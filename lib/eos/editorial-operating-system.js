...
} = require("./market-discovery");
const { buildMarketReadiness } = require("./market-readiness");
...
const MARKET_PROGRAMS = {
  ...
  marketDiscovery: {
    id: "market_discovery",
    label: "Commercial Market Discovery",
    ownerSystem: "EOS",
    purpose: "Tracks external market research and discovery-vs-canonical gap analysis without promoting findings automatically.",
    sourceSystems: ["Commercial Market Discovery", "EOS"],
  },
};
...
function buildHealthSignals(metro) {
  ...
  const recommendationQaReady = metro.compassStatus === "ready";
  const recommendationQaScore = recommendationQaReady ? recommendation.score : 0;
  const recommendationQaNote = recommendationQaReady
    ? recommendation.explanation || "Compass recommendation confidence."
    : "Authoritative Recommendation QA is not independently scoped or verified for this market yet.";
  ...
  return {
    ...
    recommendationConfidence: signal("recommendationConfidence", "Recommendation Coverage", recommendationQaScore, recommendationQaNote),
...
function buildPortfolioQueues(workQueue, metros, expansionProjects, portfolioResolution, primaryDistrictMissions = []) {
  ...
  const reservedBuildingPaths = new Set(
    (primaryDistrictMissions || [])
      .concat(buildingProfileMissions)
      .flatMap((mission) => mission.includedTasks || [])
      .map((task) => normalizePath(task.buildingPath))
      .filter(Boolean)
  );
  for (const task of editorialQueue) {
    const publicUrl = normalizePath(task && task.source && task.source.publicUrl);
    if (task.category === "buildingBriefs" && publicUrl && reservedBuildingPaths.has(publicUrl)) {
      reservedWorkItemIds.add(task.id);
    }
  }
...
function programProgressForMarket(program, metro, fieldModeItem, platformServices) {
  ...
  if (program.id === MARKET_PROGRAMS.marketDiscovery.id) return marketDiscoveryProgress(platformServices && platformServices.marketDiscovery, metro.metroId);
...
function campaignSizingStrategy(program) {
  ...
  if (program.id === MARKET_PROGRAMS.marketDiscovery.id) {
    return "Market Discovery is a research audit program. Discovery findings become implementation missions only after separate canonical promotion approval.";
  }
...
function buildMarketProjection(metros, portfolioQueues, platformServices, portfolioResolution) {
  ...
  const markets = (metros || []).map((metro) => {
    const programs = Object.values(MARKET_PROGRAMS).map((program) => {
      ...
      const progress = programProgressForMarket(program, metro, fieldModeByMetro.get(metro.metroId), platformServices);
  ...
  return {
    ...
    sourceSystems: ["Publisher", "Commercial Market Evidence", "Commercial Market Discovery", "Commercial Knowledge Graph", "Field Mode", "Compass", "EOS"],
    ownership: {
      publisher: "Determines readiness and constraints.",
      commercialMarketEvidence: "Owns evidence collections and validation.",
      marketDiscovery: "Owns external market research and discovery-vs-canonical gap analysis.",
...
function buildPlatformServices(publisherSnapshot, publisherAnalysis) {
  ...
  const marketDiscovery = buildMarketDiscoveryService({
    generatedAt: publisherAnalysis && publisherAnalysis.generatedAt,
  });
  return {
    marketDiscovery,
...
function buildEditorialOperatingSystem(publisherSnapshot, expansionPlans, options = {}) {
  ...
  return {
    ...
    sourceSystems: ["Publisher", "Compass", "Commercial Knowledge Graph", "Commercial Market Evidence", "Commercial Market Discovery", "Field Mode", "QA", "Editorial Planning"],
