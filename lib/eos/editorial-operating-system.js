...
function buildMarketProjection(metros, portfolioQueues, platformServices) {
  ...
  const markets = (metros || []).map((metro) => {
    const programs = Object.values(MARKET_PROGRAMS).map((program) => {
      ...
      const initiatives = buildInitiativesForProgram(metro, program, missions, progress);
...
function buildCommercialMarketEvidenceExpansion(publisherAnalysis) {
  ...
  return {
    ...
    planningImpact: "Read-only in v1. This expansion queue does not create executable missions or affect Current Focus.",
  ...
  return {
    commercialMarketEvidence: commercialMarketEvidence
      ? {
        ...
        planningImpact: "None in v1. Mission Control displays platform health without generating Market Evidence missions.",
        expansion,
      }
      : {
