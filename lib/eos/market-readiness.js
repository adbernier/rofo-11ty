...
function buildPropertyType(market, nodes, propertyType, marketDependenciesReady) {
  ...
  return {
    ...
    workloads: {
      spaceTypeFit: {
        status: fit,
        evidence: foundation ? [foundation.coverage?.schemaVersion?.includes("retail") ? "_data/sfRetailMarketCoverage.js" : "_data/sfOfficeMarketCoverage.js"] : nodes.some((node) => propertyType.fitKeys.some((key) => node.spaceTypeFit?.[key])) ? ["_data/locationKnowledgeGraph.js"] : [],
