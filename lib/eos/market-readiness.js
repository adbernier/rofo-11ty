...
const sfPublicExperienceCertification = require("../../_data/sfPublicExperienceCertification");
...
function publicExperience(market, nodes, foundations) {
  ...
  const sfCertification = market.marketId === "san-francisco" ? sfPublicExperienceCertification : null;
  ...
  const status = sfCertification?.status === "READY"
    ? STATUS.READY
    : !publicDistricts && !market.publicRoute
    ? STATUS.NOT_STARTED
  ...
  if (presentations.length && imageDistricts < presentations.length && !sfCertification?.photography?.blocking) {
    // Photography remains visible as finite polish, but does not block the certified SF decision journey.
  } else if (presentations.length && imageDistricts < presentations.length) gaps.push(`${presentations.length - imageDistricts} projected districts lack imagery`);
  ...
  return {
    status,
    evidence: compactEvidence([market.publicRoute ? "_data/commercialGeography.js" : "", publicDistricts ? "_data/locationKnowledgeGraph.js" : "", sfPublicSurfaceIds ? "_data/sfPublicDecisionSurfaces.js" : "", sfRepresentativeIds.length ? "_data/sfRepresentativeContent.js" : "", market.marketId === "san-francisco" ? "_data/sfPublicSampleBriefs.js" : "", sfCertification ? "_data/sfPublicExperienceCertification.js" : "", presentations.length ? "data/generated/location-brief-district-presentation.json" : ""]),
    details: { publicDistricts, projectedDistricts: presentations.length, imageDistricts, representativeBuildingDistricts: buildingDistricts, ...(market.marketId === "san-francisco" ? { certifiedSampleBriefs: sfPublicSampleBriefs.briefs.length, photographyBacklog: sfCertification.photography.missingDistrictIds.length, photographyBlocking: false, hardGatesPassed: sfCertification.hardGates.length } : {}) },
    gaps,
  };
}
