...
function meaningfulCoverage(foundation) {
  return (foundation?.coverage?.decisionGeographies || []).filter((item) =>
    ["CORE_OFFICE", "SITUATIONAL_OFFICE"].includes(item.classification)
  );
}
...
function fitStatus(nodes, propertyType, foundation) {
  if (foundation && !foundation.coverage?.blockingGaps?.length && allCoverageIs(foundation, ["officeFit"])) return STATUS.READY;
...
function publicExperience(market, nodes, foundations) {
  const publicDistricts = nodes.filter((node) => node.path).length;
  const presentations = nodes.map((node) => districtPresentation.districts?.[node.slug]).filter(Boolean);
  ...
  return {
    status,
    evidence: compactEvidence([market.publicRoute ? "_data/commercialGeography.js" : "", publicDistricts ? "_data/locationKnowledgeGraph.js" : "", presentations.length ? "data/generated/location-brief-district-presentation.json" : ""]),
...
function buildPropertyType(market, nodes, propertyType, marketDependenciesReady) {
  ...
  return {
    ...
    workloads: {
      spaceTypeFit: {
        status: fit,
        evidence: foundation ? ["_data/sfOfficeMarketCoverage.js"] : nodes.some((node) => propertyType.fitKeys.some((key) => node.spaceTypeFit?.[key])) ? ["_data/locationKnowledgeGraph.js"] : [],
...
function buildMarket(market) {
  ...
  const intelligenceReady = Boolean(audited && allCoverageIs(audited, ["officeFit", "businessEnvironment"]));
