...
function ecosystemPrompt(metro, sprint) {
  if (!sprint) return "";
  const lines = [];
  lines.push(`Implement Publisher ecosystem sprint for ${metro.metroName}: ${sprint.title}`);
  lines.push("");
  lines.push("Framework:");
  lines.push("- Businesses search for places where their operations can succeed, not for property types.");
  lines.push("- Use the Commercial Ecosystem Framework and canonical taxonomy.");
  lines.push("- Do not change recommendation rankings, Search Profile questions, or public page design.");
  lines.push("");
  lines.push(`Target ecosystem: ${sprint.ecosystemLabel} (${sprint.ecosystemId})`);
  lines.push(`Metro relevance: ${sprint.relevanceLabel}`);
  lines.push(`Current readiness: ${sprint.readinessLabel}`);
  lines.push(`Sprint family: ${sprint.sprintFamily}`);
  lines.push("");
  lines.push("Current layer coverage:");
  Object.entries(sprint.layers || {}).forEach(([layer, state]) => lines.push(`- ${layer}: ${state}`));
  lines.push("");
  lines.push("Known gaps:");
  (sprint.gaps || []).forEach((gap) => lines.push(`- ${gap}`));
  lines.push("");
  lines.push("Target subtypes:");
  lines.push(`- ${(sprint.targetSubtypes || []).join(", ") || "review required"}`);
  lines.push("Target archetypes:");
  lines.push(`- ${(sprint.targetArchetypes || []).join(", ") || "review required"}`);
  lines.push("Target business activities:");
  lines.push(`- ${(sprint.targetActivities || []).join(", ") || "review required"}`);
  lines.push("Missing representative roles:");
  lines.push(`- ${(sprint.missingRepresentativeRoles || []).join(", ") || "none identified"}`);
  lines.push("Missing operational categories:");
  lines.push(`- ${(sprint.missingOperationalCategories || []).join(", ") || "none identified"}`);
  lines.push("Existing representative buildings:");
  lines.push(`- ${(sprint.existingRepresentativeBuildings || []).map((item) => `${item.name} (${item.role || "role review required"})`).join("; ") || "none"}`);
  lines.push("District candidates:");
  lines.push(`- ${(sprint.districtCandidates || []).map((item) => item.label || item.slug).join(", ") || "review required"}`);
  lines.push("");
  lines.push("Dependencies:");
  (sprint.dependencies || []).forEach((item) => lines.push(`- ${item}`));
  lines.push("");
  lines.push("Completion criteria:");
  (sprint.completionCriteria || []).forEach((item) => lines.push(`- ${item}`));
  lines.push("");
  lines.push("Guardrails:");
  lines.push("- Do not invent districts, buildings, tenants, availability, rents, or unsupported facts.");
  lines.push("- Do not deepen office-only coverage when the target ecosystem is different.");
  lines.push("- Do not treat taxonomy compatibility as proof that a property has a feature.");
  lines.push("- Validate loading, power, parking, yards, lab, food, medical, and permitted-use claims before authoring them as facts.");
  lines.push("- Do not recommend Building Brief migration before canonical representative buildings exist.");
  lines.push("- Regenerate Publisher snapshots and run ecosystem readiness QA.");
  lines.push("");
  lines.push("Verification:");
  lines.push("- npm run publisher:snapshot");
  lines.push("- npm run publisher:report");
  lines.push("- npm run build");
  lines.push("- node scripts/qa-commercial-ecosystem-foundation.js");
  lines.push("- node scripts/qa-publisher-ecosystem-readiness.js");
  lines.push("- node scripts/qa-representative-building-intelligence.js");
  return lines.join("\n");
}

function ecosystemSprintPriority(gap) {
  const severity = { critical: 100, high: 70, medium: 40, low: 15 }[gap.severity] || 0;
  const relevance = { core: 30, important: 20, secondary: 8, specialized: 8 }[gap.relevance] || 0;
  const strategicIndex = ((rules.ecosystemReadiness || {}).strategicPriority || []).indexOf(gap.ecosystemId);
  const strategic = strategicIndex === -1 ? 0 : Math.max(0, 24 - strategicIndex * 3);
  return severity + relevance + strategic + (gap.blocksEcosystemReadiness ? 25 : 0);
}

function buildRecommendedEcosystemSprint(metro) {
  const gaps = [...(metro.ecosystemGaps || [])].sort((a, b) =>
    ecosystemSprintPriority(b) - ecosystemSprintPriority(a) ||
    a.ecosystemLabel.localeCompare(b.ecosystemLabel) ||
    a.code.localeCompare(b.code)
  );
  const topGap = gaps[0] || null;
  if (!topGap) return null;
  const evaluation = (((metro.ecosystemReadiness || {}).evaluations) || []).find((item) => item.ecosystemId === topGap.ecosystemId) || {};
  const representativeIntelligence = evaluation.representativeBuildingIntelligence || {};
  const office = (((metro.ecosystemReadiness || {}).evaluations) || []).find((item) => item.ecosystemId === "office");
  const alternative = office && topGap.ecosystemId !== "office" ? {
    title: `${metro.metroName} Office Building Brief Migration`,
    whyNotFirst: office.readinessState === "developed" || office.readinessState === "strong"
      ? "Office is already one of the metro's strongest ecosystems. Additional office depth would not improve commercial balance as much as the target ecosystem sprint."
      : "Office work remains useful, but the target ecosystem has a higher strategic gap based on current readiness.",
  } : null;
  const sprint = {
    title: `${metro.metroName} ${topGap.ecosystemLabel} ${topGap.recommendedSprintFamily}`,
    objective: `Improve ${metro.metroName} commercial balance by advancing ${topGap.ecosystemLabel} from ${evaluation.readinessLabel || topGap.readinessState} toward stronger ecosystem readiness.`,
    ecosystemId: topGap.ecosystemId,
    ecosystemLabel: topGap.ecosystemLabel,
    relevance: evaluation.relevance || topGap.relevance,
    relevanceLabel: evaluation.relevanceLabel || topGap.relevance,
    readinessState: evaluation.readinessState || topGap.readinessState,
    readinessLabel: evaluation.readinessLabel || topGap.readinessState,
    sprintFamily: topGap.recommendedSprintFamily,
    rationale: `${topGap.ecosystemLabel} is ${evaluation.relevanceLabel || topGap.relevance} for ${metro.metroName} and is currently ${evaluation.readinessLabel || topGap.readinessState}. ${topGap.rationale}`,
    gapCode: topGap.code,
    priorityScore: ecosystemSprintPriority(topGap),
    layers: evaluation.layers || {},
    counts: evaluation.counts || {},
    gaps: evaluation.gaps || [topGap.rationale],
    targetSubtypes: evaluation.targetSubtypes || [],
    targetArchetypes: evaluation.targetArchetypes || [],
    targetActivities: evaluation.targetActivities || [],
    missingRepresentativeRoles: representativeIntelligence.missingRoles || [],
    missingOperationalCategories: representativeIntelligence.missingOperationalCategories || [],
    currentRepresentativeRoles: representativeIntelligence.rolesCovered || [],
    currentOperationalCategories: representativeIntelligence.operationalCategoriesCovered || [],
    existingRepresentativeBuildings: representativeIntelligence.exampleBuildings || [],
    districtCandidates: ((metro.ecosystemCoverage || {}).ecosystems || {})[topGap.ecosystemId]
      ? (((metro.ecosystemCoverage || {}).ecosystems || {})[topGap.ecosystemId].districts || []).slice(0, 8)
      : [],
    expectedBuildingRange: topGap.recommendedSprintFamily === "Ecosystem Representative Building Foundation" ? "8-15 source-supported representative buildings, selected for role and subtype breadth rather than raw count" : "use Publisher task scope",
    dependencies: [
      "Ecosystem classification",
      "District foundation",
      "Recommendation foundation",
      "Representative Building foundation",
      "Building Brief migration after canonical records exist",
    ],
    completionCriteria: [
      ...(topGap.completionCriteria || []),
      "Every target representative role has at least one source-supported building or is explicitly marked research required",
      "Target operational categories are represented without asserting unverified property features",
      "Representative Building intelligence QA passes",
    ],
    blockingEcosystemReadiness: Boolean(topGap.blocksEcosystemReadiness),
    alternativeSprint: alternative,
  };
  sprint.codexPrompt = ecosystemPrompt(metro, sprint);
  return sprint;
}
...
function currentStateFor(metro) {
  return {
    ...
    geographicReadiness: metro.geographicReadiness ? metro.geographicReadiness.state : "",
    ecosystemReadiness: metro.ecosystemReadiness ? metro.ecosystemReadiness.state : "",
    commercialBalance: metro.ecosystemBalance ? metro.ecosystemBalance.state : "",
  };
}
...
function buildMetroPlan(metro, options = {}) {
  ...
  const recommendedEcosystemSprint = buildRecommendedEcosystemSprint(metro);
  return {
    metroId: metro.metroId,
    metroName: metro.metroName,
    ...
    geographicReadiness: metro.geographicReadiness || null,
    ecosystemReadiness: metro.ecosystemReadiness || null,
    ecosystemBalance: metro.ecosystemBalance || null,
    ecosystemGaps: metro.ecosystemGaps || [],
    recommendedEcosystemSprint,
    gaps,
    priorities,
