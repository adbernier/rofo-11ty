...
function finalizeRepresentativeIntelligenceBucket(bucket, relevance) {
  const target = bucket.representativeBuildingIntelligence;
  const coveredRoles = Object.keys(target.rolesCovered || {});
  const expectedRoles = representativeBuildingIntelligenceTaxonomy.representativeRoles
    .filter((role) => role.ecosystemId === bucket.id)
    .map((role) => role.id);
  const coveredCategories = Object.keys(target.operationalCategoriesCovered || {});
  const expectedCategories = representativeBuildingIntelligenceTaxonomy.operationalCharacteristicCategories
    .filter((category) =>
      representativeBuildingIntelligenceTaxonomy.operationalCharacteristics.some((characteristic) =>
        ...
        (characteristic.applicableEcosystems || []).includes(bucket.id)
      )
    )
...
function evaluateEcosystemLayer(metro, ecosystemId, bucket) {
  ...
  finalizeRepresentativeIntelligenceBucket(bucket, relevance);
...
function buildEcosystemGaps(evaluations, balance) {
  ...
  for (const evaluation of evaluations) {
    ...
    if ((intelligence.missingRoles || []).length && evaluation.relevance !== "not_applicable") {
    ...
    if ((intelligence.missingOperationalCategories || []).length && evaluation.counts.representativeBuildings > 0) {
  ...
  (balance.warnings || []).forEach((warning, index) => {
    const dominant = evaluations.find((item) => item.ecosystemId === balance.dominantEcosystem) || evaluations[0];
    if (!dominant) return;
    gaps.push(ecosystemGap(dominant, index === 0 ? "ecosystem_concentration" : "ecosystem_building_brief_concentration", "balance", "medium", warning, { sprintFamily: "Ecosystem Balance Sprint" }));
  });
  return gaps;
}
