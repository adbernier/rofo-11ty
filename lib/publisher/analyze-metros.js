...
function taxonomyEcosystemIds(item) {
  if (!item) return [];
  if (item.ecosystemId) return [item.ecosystemId];
  if (Array.isArray(item.ecosystemIds)) return item.ecosystemIds;
  return (item.preferredEcosystems || []).concat(item.possibleEcosystemAlternatives || []);
}

function distributeCommercialMetadata(ecosystems, ecosystem, metadataKey, taxonomyLookup) {
  const primaryBucket = ecosystems[ecosystem.primary];
  (ecosystem[metadataKey] || []).forEach((id) => {
    if (primaryBucket) incrementCounter(primaryBucket[metadataKey], id);
    const taxonomyItem = taxonomyLookup[id];
    for (const ecosystemId of taxonomyEcosystemIds(taxonomyItem)) {
      if (ecosystemId === ecosystem.primary || !(ecosystem.secondary || []).includes(ecosystemId)) continue;
      const bucket = ecosystems[ecosystemId];
      if (bucket) incrementCounter(bucket[metadataKey], id);
    }
  });
}
...
function representativeIntelligenceState(target, relevance, options = {}) {
  ...
  const targetRoles = options.expectedRoles || [];
  const targetCategories = options.expectedCategories || [];
  const targetCalibrated = Boolean(options.targetCalibrated);
  const targetRolesCovered = targetRoles.length > 0 && targetRoles.every((id) => target.rolesCovered[id]);
  const targetCategoriesCovered = targetCategories.length > 0 && targetCategories.every((id) => target.operationalCategoriesCovered[id]);
  ...
  if (targetCalibrated && targetRolesCovered && targetCategoriesCovered && activityCount >= 2 && categoryCount >= 2 && authoredRatio >= 0.5) return "strong";
  if (targetCalibrated && targetRolesCovered && categoryCount >= 1 && authoredRatio >= 0.5) return "partial";
  return "thin";
}

function hasRepresentativeIntelligenceTargetCalibration(metro, ecosystemId) {
  const targetRoles = (((rules.ecosystemReadiness || {}).targetRepresentativeRoles || {})[metro && metro.id] || {})[ecosystemId];
  const targetCategories = (((rules.ecosystemReadiness || {}).targetOperationalCategories || {})[metro && metro.id] || {})[ecosystemId];
  return (Array.isArray(targetRoles) && targetRoles.length > 0) || (Array.isArray(targetCategories) && targetCategories.length > 0);
}
...
function finalizeRepresentativeIntelligenceBucket(bucket, relevance, metro = null) {
  ...
  target.state = representativeIntelligenceState(target, relevance, {
    expectedRoles,
    expectedCategories,
    targetCalibrated: hasRepresentativeIntelligenceTargetCalibration(metro, bucket.id),
  });
...
function buildEcosystemCoverage(districts, representativeBuildings, briefs) {
  ...
  districts.forEach((district) => {
    ...
    distributeCommercialMetadata(ecosystems, ecosystem, "subtypes", commercialEcosystemTaxonomy.subtypeById);
    distributeCommercialMetadata(ecosystems, ecosystem, "activities", commercialEcosystemTaxonomy.activityById);
    distributeCommercialMetadata(ecosystems, ecosystem, "archetypes", commercialEcosystemTaxonomy.archetypeById);
...
function buildEcosystemGaps(evaluations, balance) {
  ...
  for (const evaluation of evaluations) {
    ...
    if (evaluation.readinessState === "missing" && !evaluation.counts.districts && !evaluation.counts.secondaryDistricts) {
