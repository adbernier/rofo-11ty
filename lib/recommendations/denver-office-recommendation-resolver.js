const denverOfficeRecommendationModel = require("../../_data/denverOfficeRecommendationModel");
...
function normalizeProfile(rawProfile = {}) {
  ...
  const profile = {
    ...
    operationalUse: unique([...list(rawProfile.operationalUse), ...list(facts.operationalUse)]).map(slugKey),
    ...
    freewayAccessImportance: normalizeImportance(rawProfile.freewayAccessImportance || priorities.freewayAccess || priorities.highwayAccess || ""),
...
function modelDistricts(model) {
  return model.districtOrder.map((districtId) => ({ districtId, ...model.districts[districtId] }));
}
...
function addReason(states, districtId, action, signalId, signalLabel, attributeLabel, points, practicalImplication) {
  ...
  state.reasons.push({ action, signalId, signalLabel, attributeLabel, points, practicalImplication });
  if (action === "enter") {
    state.included = true;
    state.entered = true;
  }
...
function applyPriority(model, states, priorityId, signalValue) {
...
function applyConstraintValue(states, constraint, signalId, signalLabel) {
  if (!constraint || constraint.unsupported) return false;
...
function applyOperationalUse(model, states, uses) {
  let used = false;
  uses.forEach((use) => {
    const effect = model.operationalUseEffects[use];
    if (!effect) return;
    if (effect.priorityAlias) {
      used = applyPriority(model, states, effect.priorityAlias, "high") || used;
      return;
    }
    if (effect.constraintAlias === "institutionProximity") {
      ...
      applyConstraintValue(states, constraint, "institutionProximity", "Institution proximity: healthcare services");
      used = true;
      return;
    }
...
function chooseState(candidates, signalsUsed) {
...
function markMovement(states, model) {
  for (const state of states.values()) {
    if (state.excluded) state.movement = "excluded";
    else if (state.entered) state.movement = "entered";
    else if (state.score >= SCORE_THRESHOLD.rise) state.movement = "rose";
    else if (state.score <= SCORE_THRESHOLD.fall) state.movement = "fell";
    else if (model.initialConsiderationSet.includes(state.districtId)) state.movement = "held";
    else state.movement = "secondary";
  }
}
...
function shortlistForState(stateId, candidates) {
  ...
  return candidates.filter((candidate) => candidate.score >= maxScore - DEFENSIBLE_DISTANCE);
}
...
function highestValueNextQuestion(model, profile, candidates) {
  ...
  const ranked = Object.entries(model.nextQuestions).map(([questionId, question]) => {
  ...
  const selected = ranked.find((item) => item.score > 0) || ranked.find((item) => item.score === 0);
...
function shortlistRationale(stateId, shortlist, candidates) {
  if (stateId === "starting_set") {
    return "Five default Denver office districts remain visible because only market and space type are known.";
  }
...
function unresolvedTradeoffs(profile) {
  const tradeoffs = [];
  if (!profile.commuteOrientation) tradeoffs.push("central-city versus southeast/east Denver commute orientation");
  if (!profile.officeEnvironment || profile.officeEnvironment === "not_sure") tradeoffs.push("preferred office environment");
  if (!profile.clientVisitFrequency) tradeoffs.push("client, patient, partner, or stakeholder visit frequency");
  if (!profile.expectedGrowth) tradeoffs.push("expected growth and floor-plate flexibility");
  if (!profile.businessType) tradeoffs.push("business type or industry context");
  return tradeoffs;
}

function resolveDenverOfficeRecommendation(rawProfile = {}, model = denverOfficeRecommendationModel) {
  ...
  if (profile.spaceType && slugKey(profile.spaceType) !== "office") {
    return {
      ...
      applicable: false,
      reason: "The Denver Office model only evaluates office searches.",
    };
  }
  ...
  if (applyConstraintValue(states, institutionEffect, "institutionProximity", `Institution proximity: ${profile.institutionProximity.replace(/_/g, " ")}`)) {
    usedSignals.push("institutionProximity");
  }

  if (applyPriority(model, states, "regional_transit", profile.transitImportance)) usedSignals.push("regional_transit");
  if (applyPriority(model, states, "parking", profile.parkingImportance)) usedSignals.push("parking");
  if (applyPriority(model, states, "freeway_access", profile.freewayAccessImportance)) usedSignals.push("freeway_access");
  if (applyPriority(model, states, "walkability_amenities", profile.walkabilityAmenitiesImportance)) usedSignals.push("walkability_amenities");
  if (applyPriority(model, states, "client_access", profile.clientVisitFrequency)) usedSignals.push("client_access");
  if (applyPriority(model, states, "recruiting", profile.recruitingImportance)) usedSignals.push("recruiting");
  if (applyPriority(model, states, "growth_flexibility", profile.expectedGrowth)) usedSignals.push("growth_flexibility");
  ...
  if (applyOperationalUse(model, states, profile.operationalUse)) usedSignals.push("operationalUse");
  ...
  const stateId = chooseState(currentCandidates, usedSignals);
  ...
  return {
    ...
    ignoredSignals,
    unresolvedTradeoffs: unresolvedTradeoffs(profile),
    ...
    economicsPolicy: {
      ...
      explanation: "Budget, current rents, concessions, availability, and costPosition do not influence Denver Office district recommendations.",
    },
  };
}

module.exports = {
  resolveDenverOfficeRecommendation,
  normalizeProfile,
  denverOfficeRecommendationModel,
};
