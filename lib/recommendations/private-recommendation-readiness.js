(function (root, factory) {
  ...
  function sacramentoIndustrialFlexUniverse(requirement, dependencies) {
    const composition = sacramentoIndustrialFlexComposer.composeLocationRecommendations(requirement, dependencies.sacramentoIndustrialFlexFoundation, { deferShortlist: true });
    if (!composition.supported || composition.projection?.abstention) {
      const gap = composition.projection?.abstention;
      return { composition, districts: [], gaps: gap ? [gapRecord(requirement, { districtId: "sacramento" }, gap.code, "Requirement scope", "CORE", "BLOCKS_FAIR_EVALUATION", gap.reason)] : [] };
    }
    return { composition, gaps: [], districts: composition.considered.map((item) => ({
      districtId: item.districtId, districtName: item.districtName, canonicalDistrictId: item.districtId, memberDistrictIds: item.memberDistrictIds,
      activation: [{ signal: `reviewed bounded City of Sacramento ${composition.resolvedModel} decision universe`, materiality: "CORE" }],
      dimensions: {
        canonicalGeography: { status: "REVIEWED", evidence: item.evidenceIds },
        propertyTypeFit: { status: item.compositionBand === "INELIGIBLE" ? "REVIEWED_INELIGIBLE" : "REVIEWED", band: item.propertyTypeFit.band, evidence: item.evidenceIds },
        businessEnvironment: { status: item.compositionBand === "INELIGIBLE" ? "NOT_APPLICABLE" : "REVIEWED", band: item.environment.band, evidence: item.evidenceIds },
        accessIntelligence: { status: "NOT_APPLICABLE", reason: "Access does not rank this bounded City of Sacramento universe; decisive access needs abstain." },
        operatingUse: { status: item.compositionBand === "INELIGIBLE" ? "NOT_APPLICABLE" : "REVIEWED", band: item.environment.band, evidence: item.evidenceIds },
        representativeContext: { status: item.representatives?.length ? "REVIEWED" : "UNKNOWN" },
        propertyCapability: { status: "INVESTIGATE", reason: item.unknowns?.[0] },
      },
  ...
  function evaluateRecommendationReadiness(requirement, dependencies) {
    ...
    else if (sacramentoIndustrialFlexComposer.resolveMembership(requirement).eligible && dependencies.sacramentoIndustrialFlexEnabled) result = sacramentoIndustrialFlexUniverse(requirement, dependencies);
    ...
    const sacramentoSupported = sacramentoIndustrialFlexComposer.resolveMembership(requirement).eligible && dependencies.sacramentoIndustrialFlexEnabled;
    const supportedFoundation = (market === "san-francisco" && ["office", "retail_service", "industrial_flex"].includes(property)) || (market === "san-diego" && property === "industrial_flex" && dependencies.sanDiegoIndustrialFlexEnabled) || northOrangeCountySupported || phoenixSupported || indianapolisSupported || sacramentoSupported;
    ...
    if (sacramentoSupported && readiness !== READINESS.INVESTIGATE && evaluated.length === 1) {
      readiness = READINESS.BOUNDED;
      rationale = "One bounded City of Sacramento operating environment is supported as a starting point; broader alternatives require investigation.";
    }
    const composed = readiness === READINESS.INVESTIGATE || !result.composition ? null : market === "san-diego" && property === "industrial_flex"
      ...
      : sacramentoSupported
      ? sacramentoIndustrialFlexComposer.composeLocationRecommendations(requirement, dependencies.sacramentoIndustrialFlexFoundation)
