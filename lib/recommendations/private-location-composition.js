(function (root, factory) {
  ...
  function accessComponent(access) {
    ...
    const mixedRegionalScope = materialEmployeeCohorts.some((cohort) => /across the bay area|bay area \/ mixed|mixed bay area/i.test(cohort.rawOrigin || ""));
    if (mixedRegionalScope && hasUnknownMaterialEmployee) {
      return { band: "UNKNOWN", rawBand: access.overall, confidence: access.confidence, treatment: "MIXED_REGIONAL_SCOPE_UNRESOLVED" };
    }
    if (mixedRegionalScope && materialEmployeeCohorts.length > 1 && !hasUnknownMaterialEmployee) {
      const ratings = new Set(materialEmployeeCohorts.map((cohort) => cohort.rating));
      return { band: access.overall, rawBand: access.overall, confidence: access.confidence, treatment: ratings.size > 1 ? "NO_DOMINANT_ACCESS_SOLUTION" : "MIXED_REGIONAL_SCOPE_REVIEWED" };
    }
  ...
  function businessEnvironment(accessProfile, district, candidate, businessIdentity, model) {
    ...
    const reasons = [];
    const tradeoffs = [];
    const reviewedCharacteristics = district.businessEnvironmentCharacteristics || [];
    const characteristicMatches = (businessIdentity.characteristics || []).filter((item) => reviewedCharacteristics.includes(item));
    ...
    if (characteristicMatches.length) {
      band = "STRONG";
      reasons.unshift(`${businessIdentity.label} is supported by this district's reviewed business-environment characteristics.`);
    }
  ...
  function composeLocationRecommendations(requirement, accessFoundation, compositionFoundation, sfOfficeModel, options = {}) {
    ...
    const rawConsidered = shadow.access.districtResults.map((access) => {
      ...
      const environmentActivated = (businessIdentity.characteristics || []).some((item) => (district.businessEnvironmentCharacteristics || []).includes(item));
      const eligibilitySource = access.accessEligibility.startingDistrict ? "PRODUCTION_STARTING_SET" : access.accessEligibility.accessActivated ? "SHADOW_ACCESS_ACTIVATION" : environmentActivated ? "SHADOW_ENVIRONMENT_ACTIVATION" : reviewedRecommendationCandidate ? "SHADOW_RECOMMENDATION_CANDIDATE" : "NOT_ELIGIBLE";
      const item = {
