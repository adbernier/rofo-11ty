(function (root, factory) {
  ...
  function compose(requirement, accessFoundation, retailFoundation, options = {}) {
    ...
    const raw = retailFoundation.districts.map((district) => {
      const accessResult = access.districtResults.find((item) => item.districtId === district.districtId) || null;
      ...
      const eligible = district.classification !== retailFoundation.classification.NOT_RETAIL && ["GOOD", "STRONG"].includes(retailBand);
      ...
      return {
        ...
        parkingEnvironment: accessFoundation.districtProfiles.find((item) => item.districtId === district.districtId)?.parkingEnvironment || "UNKNOWN",
        classification: district.classification, compositionBand: eligible ? (retailBand === "STRONG" && environmentBand === "STRONG" ? "STRONG_FIT" : "GOOD_FIT") : "INELIGIBLE",
        ...
        internalOrdering: { componentBands: { retail: retailBand, businessEnvironment: environmentBand, access: aBand }, candidateExcluded: true },
      };
    });
    ...
    const eligible = considered.filter((item) => item.compositionBand !== "INELIGIBLE").sort((a, b) =>
      (ORDER[b.retail.band] - ORDER[a.retail.band]) || (ORDER[b.environment.band] - ORDER[a.environment.band]) || (ORDER[b.accessComponent.band] - ORDER[a.accessComponent.band]) || a.districtId.localeCompare(b.districtId)
    );
    ...
    return {
      ...
      orderingPolicy: "Retail Fit → supported Requirement-trait alignment → reviewed Access → canonical district ID. Candidate identity never participates.",
    };
  }
