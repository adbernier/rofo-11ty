...
export const QUESTION_REGISTRY_VERSION = "requirement-questions:v1.5-medical-readiness";
export const INTERVIEW_ENGINE_VERSION = "requirement-interview-engine:v1.5-medical-readiness";
...
export const QUESTION_REGISTRY = Object.freeze([
  ...
  question({ id: "industrial.pattern", stage: "USE", prompt: "What primarily happens in the space?", answerType: "multi", priority: 100, resolverId: "industrial_pattern", applicablePropertyContexts: ["industrial_flex"], satisfiedBy: ["field:activities"], options: [option("storage", "Storage / warehouse", "Storage or warehouse", "PREFERRED", { activities: ["store"] }), option("distribution", "Shipping / distribution", "Shipping or distribution", "PREFERRED", { activities: ["receive", "ship_distribute"] }), option("manufacturing", "Assembly / manufacturing", "Assembly or manufacturing", "PREFERRED", { activities: ["make_assemble"] }), option("repair", "Repair / service", "Repair or service", "PREFERRED", { activities: ["repair_service"] }), option("contractor", "Contractor / field operations", "Contractor or field operations", "PREFERRED", { activities: ["dispatch", "operate_vehicles"] }), option("research", "R&D / technical work", "R&D or technical work", "PREFERRED", { activities: ["research_test"] })], allowOther: true, decisionRelevance: ["location", "property_fit"] }),
  ...
  question({ id: "logistics.receiving", stage: "SCALE", prompt: "What usually delivers or picks up goods here?", answerType: "single", priority: 80, resolverId: "logistics", applicableActivities: ["receive", "ship_distribute"], applicableWhen: "secondary_activity_detail_open", satisfiedBy: ["dimension:industrial.access.truck_circulation", "dimension:retail.operations.delivery_receiving"], options: [option("parcel", "Parcels or small vans", "Parcel or van deliveries", "PREFERRED"), option("box", "Box trucks", "Regular box-truck access", "REQUIRED"), option("semi", "Semis or tractor-trailers", "Semi access", "REQUIRED"), option("mixed", "A mix of vehicle types", "Mixed delivery vehicles", "REQUIRED")], allowUnknown: true, allowOther: true, decisionRelevance: ["location"] }),
...
const RESOLVERS = Object.freeze({
  business_identity(questionDefinition, answer) {
    ...
    return [criterion(questionDefinition.id, questionDefinition.dimension, raw, selected ? "PREFERRED" : "UNKNOWN", { scope: "business", rationale: selected ? "User selected a bounded Requirement business identity that maps to existing reviewed business-environment intelligence." : "User supplied unresolved business identity text; it is preserved without classification or an environment prior." })];
  },
  ...
  logistics(questionDefinition, answer) { const selected = answerValue(questionDefinition, answer); const dim = ["semi", "mixed"].includes(answer.optionId) ? "industrial.access.truck_circulation" : "retail.operations.delivery_receiving"; return [criterion(questionDefinition.id, dim, selected.raw, selected.status)]; },
...
const PREDICATES = Object.freeze({
  ...
  customer_geography_relevant: (state) => state.requirement.propertyTypes.includes("medical") || (state.requirement.activities.includes("host_visitors") && (state.requirement.criteria.some((item) => item.dimension === "office.access.client_visits" && !/rare/i.test(item.value.text)) || state.requirement.propertyTypes.includes("retail_service"))),
