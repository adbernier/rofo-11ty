(function (root, factory) { if (typeof module === "object" && module.exports) module.exports = factory(); else root.RofoRequirementNorthOrangeCountyIndustrialFlexAdapter = factory(); })(typeof self !== "undefined" ? self : this, function () {
  ...
  const CANONICAL_REGIONAL_MARKET = "orange-county";
  ...
  function resolveNorthOrangeCountyIndustrialFlexMembership(input = {}) {
    ...
    const canonicalCityId = marketId === CANONICAL_REGIONAL_MARKET && MARKETS.has(city) ? city : "";
    const compatibilityCityId = MARKETS.has(marketId) && (!city || city === marketId) ? marketId : "";
    const cityId = canonicalCityId || compatibilityCityId;
    const unsupportedCandidateId = candidateDistrictIds.find((id) => !CANDIDATES.has(id)) || "";
    let reason = "ELIGIBLE";
    if (!cityId) reason = marketId === CANONICAL_REGIONAL_MARKET && !city ? "GENERIC_ORANGE_COUNTY" : marketId === CANONICAL_REGIONAL_MARKET ? "UNSUPPORTED_ORANGE_COUNTY_CITY" : "UNSUPPORTED_MARKET";
    ...
    return Object.freeze({ eligible: reason === "ELIGIBLE", reason, marketId, city, cityId, propertyType, candidateDistrictIds: Object.freeze(candidateDistrictIds.slice()), unsupportedCandidateId, canonicalRegionalMembership: Boolean(canonicalCityId), compatibilityMembership: Boolean(compatibilityCityId) });
  }
  ...
  function project(requirement = {}) {
    const membership = resolveNorthOrangeCountyIndustrialFlexMembership(requirement);
    ...
    const marketSupported = Boolean(membership.cityId);
    const propertySupported = membership.propertyType === "industrial_flex";
    const outsideCandidate = membership.unsupportedCandidateId;
  ...
  return { VERSION, resolveIntent, resolveNorthOrangeCountyIndustrialFlexMembership, projectRequirementToNorthOrangeCountyIndustrialFlexRecommendation: project };
});
