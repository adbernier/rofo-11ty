...
const criterionValue = (requirement, id) => {
  const value = (requirement.criteria || []).find((item) => item && item.dimension === id)?.value || {};
  return cleanList(value.list).join(" · ") || String(value.text || "").trim();
};
...
function projectUniversalIntelligence(requirement = {}) {
  ...
  return {
    ...
    whatMatters: relevant.slice(0, 5).map(({ item, activatedBy }) => ({
      id: item.id, label: item.label, whyItMatters: item.whyItMatters, activatedBy,
      statedRequirement: activatedBy.filter((token) => token.startsWith("dimension:")).map((token) => criterionValue(requirement, token.slice(10))).filter(Boolean).join(" · "),
    })),
