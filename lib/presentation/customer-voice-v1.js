...
function sharedSearchWhere(items) {
  ...
  const offer = item => {
    const raw = distinction(item);
    let phrase = /^North County\b/.test(raw) ? raw : raw.replace(/^./, letter => letter.toLowerCase());
    if (/office$/i.test(phrase)) phrase += " setting";
    phrase = phrase.replace(/ district$/i, " setting");
    const article = /^(north|south|east|west) county\b/i.test(phrase) ? "" : /^[aeiou]/i.test(phrase) ? "an " : "a ";
    return `${article}${phrase}`;
  };
  if (!names.length) return ["We don't have enough information yet to recommend specific areas confidently."];
  if (items.length === 1) return [`We'd start with ${names[0]}.`, `${names[0]} is the clearest starting point because it offers ${offer(items[0])}.`];
  if (items.length === 2) return [`We'd start with ${joinNames(names)}.`, `${names[0]} offers ${offer(items[0])}, while ${names[1]} offers ${offer(items[1])}.`];
  return [
    `We'd start with ${joinNames(names)}.`,
    `They represent different ways to meet the search. ${names.map((name, index) => `${name} offers ${offer(items[index])}`).join("; ")}.`
  ];
}
function projectSharedSearchBrief({ snapshot, requirement, market }) {
  ...
  const activities = (requirement.activities || []).map(value => propertyType === "industrial_flex" && value === "host_visitors" ? "customer visits" : VALUE_LABELS[value] || String(value).replace(/_/g, " "));
  const operational = criteriaFor(requirement, item => /^(industrial\.|retail\.|office\.(?:occupancy|workplace|layout))/.test(item.dimension));
  const spaceRequirements = uniqueMeaningful(operational.filter(item => item.dimension !== "office.occupancy.peak_attendance").map(spaceRequirementPhrase).filter(Boolean), 7);
  ...
  const functionalNeeds = uniqueMeaningful([...activities, ...spaceRequirements], 8);
