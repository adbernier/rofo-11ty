...
Commercial Market Evidence is a first-class platform service. Its source data and validator own evidence quality; Publisher reports the validator summary for visibility; Mission Control displays platform health, and EOS can resolve district building-evidence work into bounded Program Missions.

Mission Control performs presence-based expansion discovery for Commercial Market Evidence by comparing Knowledge Graph districts with existing evidence collections. EOS may resolve eligible district work into an executable Program Initiative and Mission, while Publisher scoring and recommendation behavior remain separate until a focused integration sprint approves deeper use. District collections are assigned to operational markets, not merely broad Publisher metro groups, so East Bay, Peninsula, South Bay, San Francisco, Seattle, Denver, and other workspaces can remain distinct even when Publisher uses broader analysis groupings.

Commercial Market Evidence and Building Profiles remain separate canonical data products, but they are one operational editorial workflow when they explain the same district. A District Building Evidence Mission can create or validate the district collection, select representative evidence, complete selected evidence Building Profiles, run both validators, regenerate snapshots, and return one SER v1. Completed CME collections do not imply district building evidence is complete when material evidence buildings still lack adequate Building Profiles.
...
Portfolio Resolvers are the deterministic bridge between Publisher's record-level work items and Mission Control's portfolio-scale execution model. The first resolver groups Building Profile work into district or ecosystem portfolios when buildings share market ownership, source files, validation paths, and editorial purpose. District Building Evidence extends that resolver architecture by letting EOS execute Commercial Market Evidence and selected Building Profile work through one district Mission while keeping the underlying systems separate. The resolver architecture is documented in `docs/eos-portfolio-resolver.md`.
