...
(async () => {
  ...
  for (const [name, activities, summary, candidate, expectedLeader] of scenarios) {
    ...
    assert(!/reviewed evidence supports|Industrial-led applicability|operating environment/i.test(html));
  ...
  assert(one.html.includes("One area to start with"));
  ...
  assert(multipleHtml.includes("areas worth comparing"));
  ...
  for (const input of abstentions) {
    ...
    assert(!html.includes("areas worth comparing"));
