...
const entry = (city = "", candidates = []) => ({ sourceType: candidates.length ? "district" : "space_type", marketId: "orange-county", city, propertyType: "industrial_flex", candidateDistrictIds: candidates });
...
(async () => {
  for (const [marketId, city, candidate] of [["anaheim", "Anaheim", ""], ["anaheim", "Anaheim", "anaheim-canyon"], ["fullerton", "Fullerton", ""], ["fullerton", "Fullerton", "fullerton-industrial-service-area"]]) {
    ...
    assert.equal(shared.isNorthOrangeCountyIndustrialFlexEntryContext(entry(city, candidates)), true);
    assert.equal(shared.isNorthOrangeCountyIndustrialFlexRequirement(requirement(city, candidates)), true);
    assert.equal(await shared.publicEntryContextEligibleAtRuntime(env, entry(city, candidates)), true, "Production-shaped EntryContext is structurally activatable");
    ...
    assert.equal(await shared.publicEntryContextEligibleAtRuntime(offEnv, entry(city, candidates)), false, "Missing D1 activation remains denied");
  ...
  assert.equal(shared.isNorthOrangeCountyIndustrialFlexEntryContext({ sourceType: "space_type", marketId: "fullerton", propertyType: "industrial_flex", candidateDistrictIds: [] }), true, "Trusted legacy/internal city EntryContext remains compatible");
