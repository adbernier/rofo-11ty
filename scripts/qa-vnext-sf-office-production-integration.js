...
assert.equal(foundation.publicSourceAllowed({}, "city"), true, "An approved city entry cannot be rejected after interview completion.");
assert.equal(foundation.publicSourceAllowed({}, "example"), true, "Public examples are canonical controlled-v2 sources.");
...
assert(entryRoute.includes("publicEntryContextEligible"));
...
assert(requirementPage.includes("create a Location Brief"));
assert(requirementPage.includes("Create My Location Brief"));
...
assert(requirementClient.includes("vnext_creation_rejected"), "Unexpected creation rejection must be observable and fall back safely.");
const createApi = read("functions/api/location-brief-v2/create.js");
assert(createApi.includes("publicRequirementEligible"));
