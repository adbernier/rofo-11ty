...
const redirectRows = suppliedRows.map((row) => {
  ...
  const sourceHost=(()=>{try{return new URL(url).host}catch{return null}})();
...
const audit={schemaVersion:"durable-property-legacy-url-audit:v1",source:{fileName:path.basename(GSC),sha256:sha256(fs.readFileSync(GSC)),exportedRows:suppliedRows.length,reportedSearchConsoleScale:"approximately 1.69K; export contains a bounded 1,000-row sample"},summary:{totalUrls:suppliedRows.length,legacyIdUrls:redirectRows.filter((row)=>row.legacyBuildingId).length,historicalPropertyMatches:redirectRows.filter((row)=>row.historicalPropertyMatch).length,durablePilotMatches:redirectRows.filter((row)=>row.durablePilotMatchId).length,canonicalDestinationMatches:redirectRows.filter((row)=>row.currentCanonicalUrl).length,classifications:counts,liveProbeCoverage:20,offlineClassifications:suppliedRows.length-20,liveProbeResult:"20/20 returned HTTP 200 after one redirect to matching city context"},records:redirectRows};
...
const index={schemaVersion:contract.schemaVersion,generatedFrom:"deterministic repository inputs plus supplied GSC export",scope:{markets:["San Francisco","Sacramento","Indianapolis"],publicBehavior:"NONE",redirectMutation:"NONE",availability:"EXCLUDED"},summary:{durableEntities:made.length,reviewedDurableEntities:made.filter(({entity})=>entity.identityConfidence==="REVIEWED_DURABLE_ENTITY").length,gscUrlsAudited:suppliedRows.length,historicalUrlMatches:audit.summary.historicalPropertyMatches,canonicalDestinationMatches:audit.summary.canonicalDestinationMatches,redirectClassifications:counts},artifacts:[...marketArtifacts,{market:"gsc-audit",file:"gsc-legacy-url-audit.json",bytes:auditBytes.length,sha256:sha256(auditBytes)}]};
write(path.join(OUTPUT,"index.json"),index);
write(path.join(OUTPUT,"README.txt"),"Durable Property Entity Pilot v1\n\nInternal-only reviewed identities. Historical observations are provenance, never current availability. URL classifications are audit findings and do not change routes or redirects. The supplied GSC export contains 1,000 examples from an approximately 1.69K Search Console issue population. A deterministic 20-slice live sample returned HTTP 200 after one redirect to matching city context in all cases; remaining dispositions use the same repository route model and are explicitly marked offline.\n");
