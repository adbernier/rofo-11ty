...
["soma", "south-beach"].forEach((districtId) => {
  ...
  assert.equal(item.access.employeeCohortResults.find((cohort) => cohort.originRegionId === "sf-origin:north-bay").rating, "WEAK");
  assert.equal(item.accessComponent.treatment, "AS_EVALUATED");
  assert.equal(item.accessComponent.band, "WEAK");
  assert.equal(item.employeeAccessSummary.band, "WEAK");
  assert.match(item.employeeAccessSummary.label, /Weak across material origins/);
});
assert(Object.values(operator.comparison.rows.find((row) => row.id === "employee").values).every((value) => !/Not established/.test(value)), "Reviewed northern access must no longer render as missing intelligence.");
