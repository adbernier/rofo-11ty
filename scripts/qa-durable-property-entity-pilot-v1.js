...
assert.equal(index.scope.publicBehavior,"BOUNDED_LEGACY_REDIRECT_DESTINATION_IMPROVEMENT"); assert.equal(index.scope.redirectMutation,"EXACT_14_REVIEWED_DIRECT_PROPERTY_REDIRECTS"); assert.equal(index.scope.availability,"EXCLUDED");
...
assert.equal(audit.summary.liveProbeCoverage,20); assert.equal(audit.summary.classifications.REDIRECT_CHAIN,0); assert.equal(audit.summary.classifications.GOOD_DIRECT_REDIRECT,14); assert.equal(audit.summary.classifications.GOOD_CONTEXT_REDIRECT,985);
