...
assert.equal(index.scope.publicBehavior,"NONE"); assert.equal(index.scope.redirectMutation,"NONE"); assert.equal(index.scope.availability,"EXCLUDED");
...
assert.equal(audit.summary.liveProbeCoverage,20); assert.equal(audit.summary.classifications.REDIRECT_CHAIN,0); assert.equal(audit.summary.classifications.GOOD_CONTEXT_REDIRECT,999);
