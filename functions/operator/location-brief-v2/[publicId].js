...
function nextStep(readiness) {
  if (readiness === "BOUNDED") return { heading: "Now let's find the right space", action: "Continue my search →", copy: "You've identified useful starting points. Next, tell Rofo what the space itself needs to support while the broader market remains open for investigation." };
  if (readiness === "INVESTIGATE") return { heading: "Now let's find the right space", action: "Continue my search →", copy: "Next, tell Rofo what the actual space needs to support so those requirements can guide market and property investigation." };
  return { heading: "Now let's find the right space", action: "Continue my search →", copy: "You've narrowed the location search. Next, tell Rofo what the space itself needs to support so we can identify properties worth investigating." };
}
...
export function renderLocationBriefV2Page(bundle, owner, debug, options = {}) {
  ...
  <section class="lb2-search"><div><p class="lb2-eyebrow">What Rofo knows</p><h2>Your search</h2><dl>${searchRows(requirement, entryContext).map(([label, value]) => `<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join("")}</dl></div>${owner ? `<a class="lb2-button lb2-button--quiet" href="${esc(editUrl)}">Edit my search</a>` : ""}</section>
  ${hasCandidates ? candidateSection(candidates, currentSnapshot, requirement) : ""}
  ${readiness === "INVESTIGATE" ? comparisonAlternatives(currentSnapshot, requirement) : ""}
  ${readiness !== "INVESTIGATE" || priorities.length ? `<section class="lb2-guidance"><div class="lb2-section-head"><p class="lb2-eyebrow">Location guidance</p><h2>${esc(guidanceHeading)}</h2></div><p class="lb2-guidance__intro">${esc(guidanceCopy)}</p>${readiness === "INVESTIGATE" ? `<ul class="lb2-investigate-list">${priorities.map((item) => `<li>${esc(item)}</li>`).join("")}</ul>` : recommendationFocus(currentSnapshot, requirement, candidateIds)}</section>` : ""}
  ${readiness === "INVESTIGATE" ? "" : comparison(currentSnapshot)}
  <section class="lb2-next"><div class="lb2-next__panel"><div><p class="lb2-eyebrow">Next step</p><h2>${esc(next.heading)}</h2><p>${esc(next.copy)}</p></div>${publicExperience && owner && propertyContinuationSupported ? `<a class="lb2-button" href="${esc(findSpacesUrl)}" data-vnext-find-spaces>${esc(next.action)}</a>` : `<button class="lb2-button" type="button" disabled title="${esc(propertyContinuationSupported ? "Continue from the browser that owns this Location Brief" : "This property-stage continuation is currently available for San Francisco Office searches")}">${esc(next.action)}</button>`}</div></section>
  ${debug ? debugPanel(bundle) : ""}
  </main><script>(function(){var endpoint='/api/analytics/search-profile';function track(name,extra){${publicExperience ? `try{fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},credentials:'same-origin',keepalive:true,body:JSON.stringify({event_name:name,profile_version:'location-brief:v2',context:Object.assign({page_type:'location_brief_v2',page_url:location.pathname,city:'San Francisco',space_type:'Office',readiness:${JSON.stringify(readiness)},brief_id:${JSON.stringify(brief.publicId)}},extra||{}),profile:{profile_version:'location-brief:v2',space_type:'Office'},attribution:{entry_page_type:${JSON.stringify(entryContext.sourceType || '')},landing_page:${JSON.stringify(entryContext.landingPage || '')}})})}catch(error){}` : ``}}track('vnext_brief_viewed');var key='rofoLocationBriefV2Return';document.querySelectorAll('[data-brief-explore]').forEach(function(link){link.addEventListener('click',function(){try{sessionStorage.setItem(key,JSON.stringify({url:${JSON.stringify(briefUrl)},label:'Back to my Location Brief'}));}catch(error){}track('vnext_district_explored',{district:link.textContent.replace(/Explore|→/g,'').trim()});});});document.querySelectorAll('a[href*="journey=edit"]').forEach(function(link){link.addEventListener('click',function(){track('vnext_requirement_edited')})});document.querySelectorAll('[data-vnext-find-spaces]').forEach(function(link){link.addEventListener('click',function(){track('vnext_find_spaces_clicked')})});document.querySelectorAll('[data-comparison-focus-root],[data-location-focus-root]').forEach(function(root){var panels=root.querySelectorAll('[data-focus-panel]');var buttons=root.querySelectorAll('[data-focus-button]');function focus(name){panels.forEach(function(panel){panel.hidden=panel.dataset.focusPanel!==name;});buttons.forEach(function(button){button.classList.toggle('is-active',button.dataset.focusButton===name);});}buttons.forEach(function(button){button.addEventListener('click',function(){focus(button.dataset.focusButton);});});});document.querySelectorAll('[data-focus-alternative]').forEach(function(button){button.addEventListener('click',function(){var root=document.querySelector('[data-comparison-focus-root]');if(!root)return;var panel=root.querySelector('[data-focus-panel="alternative"]');var buttons=root.querySelectorAll('[data-focus-button]');root.querySelectorAll('[data-focus-panel]').forEach(function(item){item.hidden=item!==panel;});buttons.forEach(function(item){item.classList.toggle('is-active',item.dataset.focusButton==='alternative');});root.scrollIntoView({behavior:'smooth',block:'start'});});});})();</script></body></html>`;
}
