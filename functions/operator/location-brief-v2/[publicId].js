...
const { customerSentence, projectLocationBrief, projectSharedSearchBrief } = customerVoice;
...
function sharedSearchMarkup(sharedSearch, owner, editUrl, revisionId) {
  const sections = sharedSearch.sections.map((section) => `<section class="lb2-assignment__section" aria-labelledby="lb2-assignment-${esc(section.id)}"><h3 id="lb2-assignment-${esc(section.id)}">${esc(section.heading)}</h3>${(section.paragraphs || []).map(value => `<p>${esc(value)}</p>`).join("")}${section.items?.length ? `<ul>${section.items.map(value => `<li>${esc(value)}</li>`).join("")}</ul>` : ""}</section>`).join("");
  const confirmation = owner ? `<div class="lb2-confirm" data-brief-confirmation data-revision-id="${esc(revisionId)}"><div><p class="lb2-eyebrow">Your search</p><h3>Does this sound right?</h3><p data-confirmation-status>Confirm that this describes what you're looking for before putting Rofo to work.</p></div><div class="lb2-confirm__actions"><button class="lb2-button" type="button" data-confirm-brief>Yes, this sounds right</button><a class="lb2-button lb2-button--quiet" href="${esc(editUrl)}" data-change-brief>Change something</a></div></div>` : "";
  return `<section class="lb2-assignment" aria-labelledby="lb2-assignment-heading"><div class="lb2-section-head"><p class="lb2-eyebrow">Location Brief</p><h2 id="lb2-assignment-heading">${esc(sharedSearch.heading)}</h2></div><div class="lb2-assignment__grid">${sections}</div>${confirmation}</section>`;
}
...
export function renderLocationBriefV2Page(bundle, owner, debug, options = {}) {
  ...
  const sharedSearch = projectSharedSearchBrief({ snapshot: certified ? currentSnapshot : { ...currentSnapshot, shortlist: [] }, requirement, market });
  const guidanceHeading = presentation.heading;
  const guidanceCopy = presentation.intro;
  const next = certified ? { heading: "Put Rofo to work finding it", copy: "Rofo can help look for current and upcoming possibilities, work with market professionals, and confirm the details that matter. The search is not limited to today's advertised listings.", action: "Put Rofo to work →" } : presentation.next;
  ...
  const candidateIds = hasCandidates ? (candidates || []).map((item) => item.canonicalDistrictId) : [];
  const findSpacesUrl = `/property-requirement/${encodeURIComponent(brief.publicId)}`;
  const researchSpacesUrl = `/research-search/${encodeURIComponent(brief.publicId)}`;
  ...
  .lb2-assignment{background:#fff;border:1px solid #c9d8ea;border-radius:14px;padding:clamp(24px,4vw,42px);margin-bottom:34px;box-shadow:0 14px 34px rgba(15,23,42,.06)}.lb2-assignment__grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 34px}.lb2-assignment__section{min-width:0;padding:22px 0;border-top:1px solid #e2e8f0;overflow-wrap:anywhere}.lb2-assignment__section h3{font-size:.78rem;text-transform:uppercase;letter-spacing:.075em;color:#64748b;margin:0 0 9px}.lb2-assignment__section p{margin:0 0 8px}.lb2-assignment__section p:last-child{margin-bottom:0}.lb2-assignment__section ul{margin:0;padding-left:19px}.lb2-confirm{display:flex;justify-content:space-between;gap:24px;align-items:center;margin-top:12px;padding:22px;border-radius:11px;background:#eef4ff}.lb2-confirm h3{font-size:1.35rem;margin:0 0 4px}.lb2-confirm p{margin:0}.lb2-confirm__actions{display:flex;gap:9px;flex-shrink:0}.lb2-confirm.is-confirmed{background:#eaf7f0}.lb2-confirm.is-confirmed [data-confirm-brief]{background:#1b6a45;border-color:#1b6a45}.lb2-summary-column{min-width:0}.requirement-search-summary__item,.requirement-search-summary__item strong{min-width:0;overflow-wrap:anywhere}@media(max-width:900px){html,body{overflow-x:hidden}.lb2-layout{grid-template-columns:minmax(0,1fr)}.lb2-main,.lb2-summary-column{min-width:0;max-width:100%;overflow-wrap:anywhere}.lb2-summary-column{order:initial}}@media(max-width:760px){.lb2-assignment__grid{grid-template-columns:minmax(0,1fr)}.lb2-confirm{display:block}.lb2-confirm__actions{margin-top:16px;flex-direction:column}.lb2-confirm__actions .lb2-button{width:100%}}
  .lb2-building-grid article{display:grid;background:#fff;padding:11px 12px;border:1px solid #e8eef5;border-radius:8px}
  ...
  <div class="lb2-layout"><div class="lb2-main">${sharedSearchMarkup(sharedSearch, owner, editUrl, currentRevision.id)}${universal.matters}${hasCandidates ? candidateSection(candidates, currentSnapshot, requirement) : ""}
