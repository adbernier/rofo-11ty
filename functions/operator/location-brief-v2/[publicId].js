...
function comparisonRows(snapshot) {
  ...
  const environmentText = (item) => cleanStrength((item.environment?.reasons || []).find((reason) => !/^Business type:/i.test(reason))) || humanBand(item.environment?.band);
  const transitBand = (item) => {
    const order = { STRONG: 5, GOOD: 4, MODERATE: 3, MIXED: 3, WEAK: 2, LIMITED: 2, UNKNOWN: 0 };
    const modes = (item.access?.modeResults || []).filter((mode) => ["REGIONAL_TRANSIT", "FERRY"].includes(mode.mode) && mode.rating !== "UNKNOWN").sort((a, b) => (order[b.rating] || 0) - (order[a.rating] || 0));
    return modes.length ? humanBand(modes[0].rating) : "Not established";
  };
  const definitions = [
    ...
    ["Business environment", environmentText],
    ["Regional transit", transitBand],
  ...
  return definitions.map(([label, getter]) => ({ label, values: items.map(getter) })).filter((row) => row.values.some(Boolean) && (new Set(row.values).size > 1 || ["Why consider it", "Key tradeoff"].includes(row.label)));
}
...
export function renderLocationBriefV2Page(bundle, owner, debug, options = {}) {
  ...
  <div class="lb2-layout"><div class="lb2-main">${hasCandidates ? candidateSection(candidates, currentSnapshot, requirement) : ""}
