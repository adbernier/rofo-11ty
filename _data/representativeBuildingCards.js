...
const districtCards = {
  ...
  "/commercial-real-estate/CA/sunnyvale/moffett-park/": [
    ...
    card("Kifer Road flex/R&D building", "1310 Kifer Rd", "Flex / R&D", "Low-rise flex and R&D building along a practical business corridor.", "Shows the flexible technical buildings available around Moffett Park without implying current availability.", { keyword_relevance: 5, multiple_listing_references: 4 }),
    ...
    card("Java Drive business park building", "Java Dr", "Office / R&D", "Business-park building near the northern Sunnyvale employment cluster.", "Shows the campus and R&D buildings that make Moffett Park a regional alternative.", { district_consistency: 5, commercial_relevance: 5 }),
    ...
    card("Mathilda Avenue edge office", "N Mathilda Ave", "Office / flex", "Edge-of-district office building connecting Moffett Park to central Sunnyvale.", "Helps tenants understand how Moffett Park connects to nearby Sunnyvale alternatives.", { district_consistency: 4, commercial_relevance: 4 }),
  ],
  ...
  "/commercial-real-estate/CA/mountain-view/north-bayshore/": [
    ...
    card("Garcia Avenue R&D building", "Garcia Ave", "R&D / flex", "Low-rise R&D/flex building in the core employment grid.", "Adds a flexible technical-user building to the district's campus-heavy identity.", { keyword_relevance: 5, district_consistency: 5 }),
    ...
    card("Bayshore Parkway office building", "Bayshore Pkwy", "Office / R&D", "Business-park office building near the bayfront commute network.", "Shows the larger office and R&D buildings that give North Bayshore its campus feel.", { keyword_relevance: 4, commercial_relevance: 5 }),
    ...
    card("Rengstorff Avenue office edge", "N Rengstorff Ave", "Office / service commercial", "Edge-of-district commercial building connecting North Bayshore to central Mountain View.", "Shows the access tradeoff tenants weigh against Downtown Mountain View.", { commercial_relevance: 4, district_consistency: 4 }),
  ...
  "/commercial-real-estate/CA/redwood-city/downtown-redwood-city/": [
    ...
    card("El Camino Real commercial edge", "El Camino Real", "Service commercial / office", "Commercial-edge building where downtown Redwood City meets regional arterial access.", "Shows how a tenant can balance walkable downtown identity with broader Peninsula access.", { keyword_relevance: 4, commercial_relevance: 4 }, { keyword_tags: ["service", "office", "El Camino"] }),
    ...
    card("Winslow Street commercial building", "Winslow St", "Office / service commercial", "Secondary-street commercial building near the downtown core.", "Shows the quieter office and local service locations a tenant may evaluate near Broadway.", { district_consistency: 4, commercial_relevance: 4 }, { keyword_tags: ["office", "service", "downtown"] }),
    card("Middlefield Road commercial building", "Middlefield Rd", "Service commercial / office", "Commercial building connecting downtown Redwood City with surrounding Peninsula neighborhoods.", "Adds local-service and small-office context to the district's downtown identity.", { keyword_relevance: 4, district_consistency: 4 }, { keyword_tags: ["service", "office", "local business"] }),
  ],
  ...
  "/commercial-real-estate/CA/emeryville/emeryville-commercial-core/": [
    ...
    card("5760 Christie Avenue", "5760 Christie Ave", "Office / mixed commercial", "Christie Avenue commercial building in Emeryville's central office and service-business area.", "Shows the practical office and service-commercial buildings near the Bay Bridge side of the East Bay.", { listing_frequency: 4, district_consistency: 5, commercial_relevance: 4 }, { keyword_tags: ["office", "mixed commercial", "Christie"] }),
    ...
    card("1603 Powell Street", "1603 Powell St", "Office / service commercial", "Smaller Powell Street commercial building near the district's core.", "Adds a smaller Emeryville building alongside the district's larger office buildings.", { district_consistency: 5, commercial_relevance: 4 }, { keyword_tags: ["office", "service", "Powell"] }),
    ...
    card("Shellmound Street commercial building", "Shellmound St", "Office / retail support", "Commercial building near Emeryville's retail and office spine.", "Adds context for the mixed-use buildings around the city's office and biotech-adjacent users.", { district_consistency: 4, commercial_relevance: 4 }, { keyword_tags: ["office", "retail", "Shellmound"] }),
    ...
    card("Landregan Street flex building", "Landregan St", "Flex / light industrial", "Smaller flex-commercial building in Emeryville's industrial-transition area.", "Shows the practical production-adjacent space that supports creative, maker, and R&D users.", { keyword_relevance: 5, district_consistency: 4 }, { keyword_tags: ["flex", "light industrial", "maker"] }),
    card("Adeline Street commercial edge", "Adeline St", "Service commercial / office", "Commercial-edge building connecting Emeryville with Berkeley and Oakland corridors.", "Useful for understanding corridor-based buildings near the Emeryville and Berkeley edge.", { district_consistency: 4, commercial_relevance: 4 }, { keyword_tags: ["service", "office", "Berkeley edge"] }),
  ],
  ...
  "/commercial-real-estate/CA/berkeley/downtown-berkeley/": [
    ...
    card("Shattuck Avenue office building", "Shattuck Ave", "Office / service commercial", "Transit-oriented office and service-commercial building in the downtown core.", "Shows the customer-facing and professional-services layer around Downtown Berkeley BART.", { keyword_relevance: 4, commercial_relevance: 5 }, { keyword_tags: ["office", "BART", "service"] }),
    ...
    card("Kittredge Street commercial building", "Kittredge St", "Office / retail support", "Secondary downtown commercial building near the main office and transit spine.", "Adds the smaller mixed commercial texture that keeps Downtown Berkeley distinct from campus-only locations.", { keyword_relevance: 4, district_consistency: 4 }, { keyword_tags: ["office", "retail", "downtown"] }),
  ],
  ...
  "/commercial-real-estate/CA/berkeley/west-berkeley/": [
    ...
    card("Gilman Street industrial building", "Gilman St", "Industrial / flex", "Industrial and flex building along a defining West Berkeley corridor.", "Shows the service, production, and R&D-adjacent buildings that often make West Berkeley different from Emeryville.", { keyword_relevance: 5, commercial_relevance: 5 }, { keyword_tags: ["industrial", "flex", "production"] }),
    ...
    card("Eighth Street flex building", "8th St", "Flex / service commercial", "Smaller flex-commercial building near West Berkeley's industrial grid.", "Adds practical service-business and production buildings beyond office-only choices.", { keyword_relevance: 4, commercial_relevance: 4 }, { keyword_tags: ["flex", "service", "production"] }),
    ...
    card("Ashby Avenue industrial edge", "Ashby Ave", "Flex / light industrial", "Industrial-edge building with regional access across Berkeley and Emeryville.", "Shows the access-oriented side of West Berkeley's commercial geography.", { keyword_relevance: 4, commercial_relevance: 4 }, { keyword_tags: ["flex", "access", "industrial"] }),
    card("University Avenue west-side commercial", "University Ave", "Office / service commercial", "West-side commercial building connecting Berkeley's downtown and waterfront-oriented districts.", "Adds a bridge between university-adjacent office demand and West Berkeley's flex environment.", { district_consistency: 4, commercial_relevance: 4 }, { keyword_tags: ["office", "service", "university"] }),
    ...
    card("West Berkeley warehouse conversion", "West Berkeley", "Adaptive warehouse / creative office", "Warehouse-conversion building supporting creative office, studio, and R&D-adjacent businesses.", "Helps describe the district's hybrid identity without implying current availability at a specific listing.", { building_prominence: 3, keyword_relevance: 5, commercial_relevance: 5 }, { keyword_tags: ["adaptive reuse", "warehouse", "creative office"] }),
  ],
