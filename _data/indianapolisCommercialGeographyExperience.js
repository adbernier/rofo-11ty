"use strict";
const evidence=require("./indianapolisIndustrialFlexEvidenceFoundation");
const durable=require("../data/internal/durable-property-entity-pilot-v1/indianapolis.json");
const editorial=Object.fromEntries(["office","retail","industrial","flex"].map(type=>[type,require(`../data/internal/indianapolis-commercial-district-intelligence-v1/${type}.json`).districts]));
...
const intros={office:"Compare Indianapolis’s mile-square business core with its dispersed northside office corridors.",retail:"Compare urban dining districts, neighborhood village retail and a regional northside commercial node.",industrial:"Compare Indianapolis’s reviewed Airport Logistics and Park 100 districts with the Park Fletcher / Stout Field component context.",flex:"Compare Park 100’s multi-tenant mix with the heavier operating context around Airport Logistics."};
const typeLabels={OFFICE_DISTRICT:"Office district",OFFICE_CORRIDOR:"Office corridor",INDUSTRIAL_DISTRICT:"Industrial district",RETAIL_DISTRICT:"Retail district",MIXED_COMMERCIAL_DISTRICT:"Mixed commercial district",FLEX_BUSINESS_PARK_ENVIRONMENT:"Flex / business-park area",LOGISTICS_ENVIRONMENT:"Logistics district"};
...
function repsFor(geography,type){
  ...
  return (candidate?.representatives||[]).slice(0,3).map(rep=>{
    ...
    return {id:rep.id,kind:rep.kind==="COMMERCIAL_ENVIRONMENT"?"ENVIRONMENT":"PROPERTY",name:rep.label,address:rep.kind==="COMMERCIAL_ENVIRONMENT"?"":(entity?.canonicalAddress||rep.label),propertyType:labels[type],geography:geography.label,municipality:"Indianapolis",canonicalUrl:rep.path||"",image:"",areaPatterns:geography.commonHere,propertyVerified:"",investigate:"",availabilityBoundary:rep.availabilitySemantics};
  });
}
const bySpaceType=Object.fromEntries(Object.keys(labels).map(type=>{
  ...
  return [type,{id:type,label:labels[type],slug:slugs[type],path:`/commercial-real-estate/IN/indianapolis/${slugs[type]}/`,explorationTitle:titles[type],introduction:intros[type],geographies}];
}));
const byGeographyId={};
...
module.exports=Object.freeze({schemaVersion:"indianapolis-public-commercial-geography-experience:v1",city:{id:"indianapolis",label:"Indianapolis",state:"IN",path:"/commercial-real-estate/IN/indianapolis/"},sourceSurfacePrefix:"indianapolis",spaceTypes:Object.values(bySpaceType),bySpaceType,byGeographyId,availabilityFirewall:"Confirm current availability and the exact condition, configuration, access, and permitted use of any specific space."});
